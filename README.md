...
|             ![위치추출](docs/images/ticket-share.gif)             |            ![타임라인](/docs/images/ticket-share.gif)             |
| :---------------------------------------------------------------: | :---------------------------------------------------------------: |
| **친구에게 여행 공유** <br> 여행 티켓 공유 후 수락/거절 선택 가능 | **실시간 공동 편집** <br> 여러 명이 동시에 사진 추가 및 수정 작업 |

### 3. 🎬 인터랙티브 여행 경로 재현

사진 촬영 시간순으로 실제 여행 경로를 자동 생성하고, 캐릭터가 물리 법칙에 따라 움직입니다.

|                 ![경로 애니메이션](docs/images/3step-image-upload.gif)                 |                   ![포토카드](docs/images/3step-image-upload.gif)                   |                ![개별 핀포인트](docs/images/3step-image-upload.gif)                 |
| :------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------: |
| **거리 기반 애니메이션** <br> 이동 거리에 따른 캐릭터 속도와<br>지도 줌 레벨 자동 조절 | **핀포인트별 포토카드** <br> 각 지점 도착 시 해당 위치 사진들을<br>카드 형태로 표시 | **동적 클러스터링** <br> 줌 확대 시 개별 사진 핀포인트로<br>분리하여 상세 위치 확인 |

### 4. 📅 위치·날짜별 사진 관리 & 보기

메타데이터(위치/날짜) 수정부터 위치와 날짜를 기준으로 사진을 볼 수 있습니다.

|      ![메타데이터 편집](docs/images/3step-image-upload.gif)      |        ![듀얼뷰 갤러리](docs/images/3step-image-upload.gif)        | ![위치별 슬라이드](docs/images/3step-image-upload.gif)  |
| :--------------------------------------------------------------: | :----------------------------------------------------------------: | :-----------------------------------------------------: |
| **메타데이터 편집** <br> 위치/날짜 없는 사진 정보를<br>직접 수정 | **날짜별 듀얼뷰** <br> 상단 지도 + 하단 시간순<br>갤러리 동시 표시 | **위치별 슬라이드** <br> 특정 장소 사진들만 모은 갤러리 |
